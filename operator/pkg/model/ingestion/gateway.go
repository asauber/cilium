...
func GatewayAPI(log *slog.Logger, input Input) *model.Model {
	...
	for _, l := range listeners {
		...
		case gatewayv1.HTTPProtocolType, gatewayv1.HTTPSProtocolType, gatewayv1.TLSProtocolType:
			...
			// (ajs) Note well, we are using the existance of AllowedNamespace
		...
		case gatewayv1.TCPProtocolType:
			resL4 = append(resL4, model.L4Listener{
				...
				Routes:         toTCPRoutes(l.Listener, input.TCPRoutes, input.Services, input.ServiceImports, input.ReferenceGrants),
				Infrastructure: infra,
				Service:        toServiceModel(input.GatewayClassConfig),
			})
		...
		case gatewayv1.UDPProtocolType:
			resL4 = append(resL4, model.L4Listener{
				...
				Routes:         toUDPRoutes(l.Listener, input.UDPRoutes, input.Services, input.ServiceImports, input.ReferenceGrants),
				Infrastructure: infra,
				Service:        toServiceModel(input.GatewayClassConfig),
			})
		}

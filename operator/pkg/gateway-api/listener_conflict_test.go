...
func Test_filterOutConflictedListeners_rejectedListenerSetEntryDoesNotConflictWithinSource(t *testing.T) {
	gatewaySource := model.FullyQualifiedResource{Kind: "Gateway"}
	listenerSetSource := model.FullyQualifiedResource{Kind: "ListenerSet", Name: "one"}
	listeners := []ingestion.ListenerWithContext{
		{
			Listener: *muxedListener("gateway", gatewayv1.HTTPProtocolType, 80, "gateway.example.test"),
			Source:   gatewaySource,
		},
		{
			Listener: *muxedListener("valid", gatewayv1.HTTPProtocolType, 80, "valid.example.test"),
			Source:   listenerSetSource,
		},
		{
			Listener: *l4Listener("conflicted", gatewayv1.TCPProtocolType, 80),
			Source:   listenerSetSource,
		},
		{
			Listener: *muxedListener("also-valid", gatewayv1.HTTPProtocolType, 80, "also-valid.example.test"),
			Source:   listenerSetSource,
		},
	}

	conflicts := conflictsAcrossSources(listeners)
	filtered := filterOutConflictedListeners(listeners, conflicts)

	assert.Equal(t, gatewayv1.ListenerReasonProtocolConflict, conflicts[listenerSetSource]["conflicted"].reason)
	assert.Len(t, conflicts[listenerSetSource], 1)
	assert.Equal(t, []gatewayv1.SectionName{"gateway", "valid", "also-valid"}, []gatewayv1.SectionName{
		filtered[0].Name,
		filtered[1].Name,
		filtered[2].Name,
	})

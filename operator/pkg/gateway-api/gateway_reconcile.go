...
func (r *gatewayReconciler) setListenerStatus(ctx context.Context, gw *gatewayv1.Gateway, httpRoutes *gatewayv1.HTTPRouteList, tlsRoutes *gatewayv1.TLSRouteList, grpcRoutes *gatewayv1.GRPCRouteList, namespaceLabels helpers.NamespaceLabelIndex) (bool, error) {
	...
	for _, l := range gw.Spec.Listeners {
		...
		if conflictMessage, ok := conflictedListeners[l.Name]; ok {
			conds = merge(conds, listenerConflictedCondition(gw, gatewayv1.ListenerReasonProtocolConflict, conflictMessage))
		...
		if !isValid {
			conds = merge(conds,
				listenerAcceptedCondition(gw.GetGeneration(), false, invalidReason, "Listener not valid. "+strings.Join(invalidMessages, " ")),
...
// runCommonRouteCheckVjjs runs all the checks that are common across all supported Route types.
//
...
func (r *gatewayReconciler) runListenerSetRouteChecks(input routechecks.Input, parent gatewayv1.ParentReference, objNamespace string) error {
	...
	hasMatchingControllerFn := helpers.GatewayHasMatchingControllerFn(context.Background(), r.Client, defaultControllerName, r.logger)
